...
# For me
plt.style.use('/home/b/bsavard/mavab/.config/matplotlib/article_figure.mplstyle')
...
def gradient2d(data, dx):
    ...
    for i in range(data.shape[0]):
        grad_x[i,:] = np.gradient(data[i,:], dx)

    grad_y = np.zeros(data.shape)
    for j in range(data.shape[1]):
        grad_y[:,j] = np.gradient(data[:,j], dx)
